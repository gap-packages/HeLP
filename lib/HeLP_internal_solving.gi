...
# the two functions are place holders. In case NormalizInterface and 4ti2Interface can be loaded they replace the corresponding function
#HeLP_INTERNAL_TestSystem_Normaliz := function()
#Info( HeLP_Info, 1, "You have accessed an internal function which should be invisible: NormalizInterface has been loaded, but HeLP_INTERNAL_TestSystem_Normaliz is not correctly defined.");
#return fail;
#end;

#HeLP_INTERNAL_TestSystem_4ti2 := function()
#Info( HeLP_Info, 1, "You have accessed an internal function which should be invisible: 4ti2Interface has been loaded, but HeLP_INTERNAL_TestSystem_4ti2 is not correctly defined.");
#return fail;
#end;
...
# It relies on the 4ti2-interface and program or on NormalizInterface and its programs. 

if HeLP_settings[1] = "normaliz" then
  if TestPackageAvailability("NormalizInterface") = fail then
    Error("Normaliz is specified as a solver, but I could not load NormalizInterface.");
  else
    return HeLP_INTERNAL_TestSystem_Normaliz(T,a,k,pa);
  fi;
elif HeLP_settings[1] = "4ti2" then
  if TestPackageAvailability("4ti2Interface") = fail then
    Error("4ti2 is specified as a solver, but I could not load 4ti2Interface.");
  else
    if IO_FindExecutable( "zsolve" ) = fail then
      Error("4ti2 is specified as a solver, but I could not find the executable zsolve.");
    else
      return HeLP_INTERNAL_TestSystem_4ti2(T,a,k,pa);
    fi;
  fi;
else 
  Info( HeLP_Info, 1, "No solver specified. Please set HeLP_settings[1] to a string containing the text 4ti2 or the text normaliz.");
  return fail;
fi;
end);
...
# ord is the order of the unit in question
# pas list of partial augmentations of the powers
local W, intersol, tab, UCT, nonintclas, o, posconk, d, posconkint;

nonintclas := HeLP_INTERNAL_DetermineInterestingClasses(CAct, ord, pas); # this is new and so posconk which goes to MakeSystem changes
o := OrdersClassRepresentatives(CAct);
posconk := [ ];
for d in DivisorsInt(ord) do
  if d <> 1 then
    Append(posconk, Positions(o, d));
  fi;
od;
...
W := HeLP_v4_INTERNAL_MakeSystem(Irr(CAct), ord, CAct, pas, posconkint);
...
#################################################################
InstallGlobalFunction("HeLP_v4_INTERNAL_WithGivenOrderAllTables", function(CAct, tables, ord)
# arguments: CAct character table
# tables: list of other character tables (derived from CAct) that should be used
# ord is the order of the unit in question
local properdivisors, d, presol, W, asol, primediv, p, npa, size_npa, j, pa, intersol, tab;
...
for d in properdivisors do
  if not IsBound(HeLP_sol[d]) then
    Info(HeLP_Info, 4, "    Solutions for order ", d, " not yet calculated.  Restart for this order.");
    presol := HeLP_v4_INTERNAL_WithGivenOrderAllTables(CAct, tables, d);
    if presol = "infinite" then
      # this should never happen
      Info( HeLP_Info, 1, "There are infinitely many solutions for elements of order ", d, ", HeLP stopped.");
      return "infinite";
...
for pa in npa do
  ...
  intersol := HeLP_v4_INTERNAL_WithGivenOrderAndPAAllTables(CAct, tables, ord, pa);
  if intersol = "infinite" then
      return "infinite";
  fi;
  Append(asol, intersol);
