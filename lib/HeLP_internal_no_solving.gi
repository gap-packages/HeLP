...
BindGlobal("HeLP_settings", [ "none", false, "32", "default"]); # Normally redund does not speed up normaliz-computations, so it is set to false
...
# Arguments: a matrix and a vector for a system of linear inequalities from which redund  should remove redundant lines
# Output: Same system of inequalities with redundant lines removed or "nosystem" in case redund already finds that there are no solutions to the system
...
BindGlobal("HeLP_INTERNAL_PPartGroupElement", function(g, p)
local ord, divp, q, i;
ord := Order(g);
...
# Hertweck's p-adic conjugacy criterion [Hertweck's unpublished preprint, reproduced in arXiv:1706.02117 ]
...
#              gp := HeLP_INTERNAL_PPartGroupElement(g, p);
